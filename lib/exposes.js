...
exports.options = {
    ...
    precision: (name) => new Numeric(`${name}_precision`, exports.access.SET).withValueMin(0).withValueMax(3).withDescription(`Number of digits after decimal point for ${name}, takes into effect on next report of device. This option can only decrease the precision, not increase it.`),
...
exports.presets = {
    ...
    auto_off: (offTime) => new Binary('auto_off', exports.access.ALL, true, false).withLabel('Auto OFF').withDescription(`Turn the device automatically off when attached device consumes less than 2W for ${offTime} minutes`).withCategory('config'),
    ...
    away_preset_temperature: () => new Numeric('away_preset_temperature', exports.access.STATE_SET).withUnit('°C').withDescription('Away preset temperature').withValueMin(-10).withValueMax(35).withCategory('config'),
    battery: () => new Numeric('battery', exports.access.STATE).withUnit('%').withDescription('Remaining battery in %, can take up to 24 hours before reported').withValueMin(0).withValueMax(100).withCategory('diagnostic'),
    battery_low: () => new Binary('battery_low', exports.access.STATE, true, false).withDescription('Indicates if the battery of this device is almost empty').withCategory('diagnostic'),
    ...
    consumer_connected: () => new Binary('consumer_connected', exports.access.STATE, true, false).withDescription('Indicates whether a plug is physically attached. Device does not have to pull power or even be connected electrically (state of this binary switch can be ON even if main power switch is OFF)').withCategory('diagnostic'),
    ...
    detection_interval: () => new Numeric('detection_interval', exports.access.ALL).withValueMin(2).withValueMax(65535).withUnit('s').withDescription('Time interval between action detection.').withCategory('config'),
    ...
    fan: () => new Fan(),
    flip_indicator_light: () => new Binary('flip_indicator_light', exports.access.ALL, 'ON', 'OFF').withDescription('After turn on, the indicator light turns on while switch is off, and vice versa').withCategory('config'),
    ...
    led_disabled_night: () => new Binary('led_disabled_night', exports.access.ALL, true, false).withLabel('LED disabled night').withDescription('Enable/disable the LED at night').withCategory('config'),
    ...
    light_color_options: () => new Composite('color_options', 'color_options', exports.access.ALL).withDescription('Advanced color behavior')
        ...
        .withCategory('config'),
    linkquality: () => new Numeric('linkquality', exports.access.STATE).withUnit('lqi').withDescription('Link quality (signal strength)').withValueMin(0).withValueMax(255).withCategory('diagnostic'),
    ...
    motion_sensitivity_select: (motion_sensitivity_names) => new Enum('motion_sensitivity', exports.access.ALL, motion_sensitivity_names).withDescription('Select motion sensitivity to use').withCategory('config'),
    ...
    overload_protection: (min, max) => new Numeric('overload_protection', exports.access.ALL).withUnit('W').withValueMin(min).withValueMax(max).withDescription('Maximum allowed load, turns off if exceeded').withCategory('config'),
    ...
    power_on_behavior: (values = ['off', 'previous', 'on']) => new Enum('power_on_behavior', exports.access.ALL, values).withLabel('Power-on behavior').withDescription('Controls the behavior when the device is powered on after power loss. If you get an `UNSUPPORTED_ATTRIBUTE` error, the device does not support it.').withCategory('config'),
    power_outage_count: (resetsWhenPairing = true) => new Numeric('power_outage_count', exports.access.STATE).withDescription('Number of power outages' + (resetsWhenPairing ? ' (since last pairing)' : '')).withCategory('diagnostic'),
    power_outage_memory: () => new Binary('power_outage_memory', exports.access.ALL, true, false).withDescription('Enable/disable the power outage memory, this recovers the on/off mode after power failure').withCategory('config'),
    ...
    schedule: () => new Binary('schedule', exports.access.ALL, true, false).withDescription('When enabled, the device will change its state based on your schedule settings').withCategory('config'),
    ...
    door_state: () => new Enum('door_state', exports.access.STATE, ['open', 'closed', 'error_jammed', 'error_forced_open', 'error_unspecified', 'undefined']).withDescription('State of the door'),
    ...
    valve_detection_bool: () => new Binary('valve_detection', exports.access.ALL, true, false).withDescription('Determines if temperature control abnormalities should be detected').withCategory('config'),
    ...
    window_detection: () => new Switch().withLabel('Window detection').withState('window_detection', true, 'Enables/disables window detection on the device', exports.access.STATE_SET), // left for compatability, do not use
    window_detection_bool: () => new Binary('window_detection', exports.access.ALL, true, false).withDescription('Enables/disables window detection on the device').withCategory('config'),
    ...
    identify_duration: () => new Numeric('identify', exports.access.SET).withValueMin(0).withValueMax(30).withUnit('seconds').withDescription('Duration of flashing').withCategory('config'),
};
